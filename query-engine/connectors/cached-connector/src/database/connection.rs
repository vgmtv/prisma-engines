...
impl<'conninfo, C> Connection for CachedConnection<'conninfo, C>
where
    C: QueryExt + TransactionCapable + Send + Sync + 'static,
{
    fn start_transaction<'a>(&'a self) -> IO<'a, Box<dyn Transaction<'a> + 'a>> {
        ...
        IO::new(self.catch(async move {
            ...
            Ok(
                Box::new(CachedConnectorTransaction::new(self.cache, tx, connection_info))
                    as Box<dyn Transaction<'a> + 'a>,
            )
        }))
    }
}

impl<'a, C> ReadOperations for CachedConnection<'a, C>
where
    C: QueryExt + Send + Sync + 'static,
{
...
impl<'conn, C> WriteOperations for CachedConnection<'conn, C>
where
    C: QueryExt + Send + Sync + 'static,
{
