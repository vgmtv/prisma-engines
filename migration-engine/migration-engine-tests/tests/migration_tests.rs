...
async fn explicit_backrelation_with_no_implicit_forward_relation_work(api: &TestApi) -> TestResult {
    let dm = r#"
        datasource db {
            provider = "postgresql"
            url      = "postgres://unreachable/unreachable"
        }
...
#[test_each_connector(tags("postgres"), log = "debug")]
async fn this_other_datamodel_works(api: &TestApi) -> TestResult {
    let dm = r#"
    datasource db {
        provider = "postgresql"
        url      = "postgres://unreachable/unreachable"
